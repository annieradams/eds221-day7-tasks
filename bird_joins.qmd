...
```{r}
bird_observations<- read.csv(here("data","52_pp52_birds_1.csv"))
...
I expect it to have 15 columns
...
```{r}
birds_full <- left_join(bird_observations, taxalist, by = "species_id") %>% 
...
```{r}
birds_full2<- birds_full %>% mutate(survey_month = lubridate::month(survey_date))
```
...
```{r}
birds_full3<- birds_full2 %>% dplyr::relocate(survey_month,.after = survey_date)
```

-   Find the total number of birds observed by *park* and *month* (i.e., you'll `group_by(park_name, survey_month)`)

```{r}
birds_full4<- birds_full3 %>% group_by(park_name,survey_month) %>% 
  summarize(count= n())
```
...
```{r}
birds_full5<- birds_full4 %>% filter(park_name %in% c("Lindo","Orme","Palomino","Sonrisa"))
```
